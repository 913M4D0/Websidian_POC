...
export const clusters: MemoryCluster[] = [
  { id: 'identity', label: 'IDENTITY', korean: '인증 · 세션', color: '#4ec9b0', glow: 'rgba(78,201,176,.24)', lane: 0, modules: ['auth-api', 'session-core', 'access-policy'] },
  { id: 'payment', label: 'PAYMENT', korean: '결제 · 정산', color: '#c586c0', glow: 'rgba(197,134,192,.22)', lane: 1, modules: ['balance-query', 'settlement-core', 'payment-gateway'] },
  { id: 'order', label: 'ORDER', korean: '주문 · 체결', color: '#ce9178', glow: 'rgba(206,145,120,.22)', lane: 2, modules: ['order-api', 'execution-core', 'trade-event'] },
  { id: 'data', label: 'DATA', korean: '데이터 · 배치', color: '#569cd6', glow: 'rgba(86,156,214,.22)', lane: 3, modules: ['warehouse-etl', 'market-batch', 'reporting'] },
  { id: 'platform', label: 'PLATFORM', korean: '공통 플랫폼', color: '#b5cea8', glow: 'rgba(181,206,168,.22)', lane: 4, modules: ['cache-sdk', 'event-sdk', 'config-core'] },
  { id: 'infra', label: 'INFRA', korean: '인프라 · 배포', color: '#dcdcaa', glow: 'rgba(220,220,170,.22)', lane: 5, modules: ['kubernetes', 'release-pipeline', 'network'] },
  { id: 'frontend', label: 'EXPERIENCE', korean: '프론트 · UX', color: '#9cdcfe', glow: 'rgba(156,220,254,.22)', lane: 6, modules: ['trading-web', 'account-app', 'design-system'] },
  { id: 'observability', label: 'OPERATIONS', korean: '관측 · 장애', color: '#d16969', glow: 'rgba(209,105,105,.22)', lane: 7, modules: ['alerting', 'incident-runbook', 'telemetry'] },
];
...
export const storyChapters = [
  ...
  { id: 'story-resolution', step: '07', label: 'HISTORY BRIEF', title: '필요 맥락 완성', caption: '나머지 기록은 배경 레이어에 유지하고 필요한 히스토리만 Brief로 컴파일합니다.' },
];
...
function nodeColor(kind: MemoryKind, clusterColor: string) {
  if (kind === 'query') return '#f3f3f3';
  if (kind === 'incident') return '#f85149';
  if (kind === 'decision') return '#c586c0';
  if (kind === 'resolution') return '#2ea043';
  return clusterColor;
}
