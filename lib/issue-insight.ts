    ...
    messages: [
      {
        ...
        content: [
          ...
          kind === 'analysis'
            ? 'Use this delimiter format exactly: write the summary text directly after <<요약>> with no nested field markers; repeat <<확인된맥락>> blocks with exactly <<제목>>, <<구분>>(사실 or 추정), <<근거>>(comma-separated allowed IDs), <<내용>> in that order; output every <<주의할위험>> block next, then output every <<권장처리>> block, using those same four fields in that order; write each question directly after <<추가확인>> with no nested field markers; finish with <<끝>>. Never put <<제목>>, <<구분>>, <<근거>>, or <<내용>> inside <<요약>> or <<추가확인>>. Produce exactly 3 맥락, at most 2 risks, exactly 3 recommendations, and at most 2 questions.'
            : 'Use this delimiter format exactly: write strategy text directly after <<전략>> with no nested field markers; exactly 3 <<테스트케이스>> blocks containing exactly <<번호>>, <<제목>>, <<우선순위>>(필수, 높음, or 보통), <<사전조건>>(at most 2 lines), <<실행단계>>(3 to 5 lines), <<기대결과>>, <<근거>>(comma-separated allowed IDs) in that order; then at most 2 <<회귀범위>> blocks containing exactly <<제목>>, <<구분>>, <<근거>>, <<내용>> in that order; finish with <<끝>>. Never put test-case fields inside <<전략>>.',
          kind === 'analysis'
